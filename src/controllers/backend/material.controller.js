...
exports.create = async (request, response) => {
  ...
  try {
    const result = await new MaterialModel(dataSave).save();
    const data = {
      _status: true,
...
exports.view = async (request, response) => {
  ...
  try {
    const totalRecords = await MaterialModel.find(filter).countDocuments();
...
exports.details = async (request, response) => {
  try {
    const result = await MaterialModel.findOne({ _id: request.params.id });
    if (result) {
      const data = {
        _status: true,
        _message: "Record Fetched",
